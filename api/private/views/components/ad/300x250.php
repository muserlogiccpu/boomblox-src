...
if (!isset($display->isFallback)) {
	...
	if (Server::isPost()) {
		if (!isset($_POST['ctl00$cphRoblox$TabbedInfo$GamesTab$RefreshRunningGamesButton']) && !empty($_POST['__EVENTARGUMENT'])) {
			if (UserAd::exists((int)$_POST['__EVENTARGUMENT']) && $_POST['__EVENTTARGET'] == 'ctl00$cphRoblox$BigUglyAd$UserAdDisplay$AdImage') {
	...
	if (Server::isPost()) {
		if (!isset($_POST['ctl00$cphRoblox$TabbedInfo$GamesTab$RefreshRunningGamesButton']) && $_POST['__EVENTARGUMENT'] == "164" && $_POST['__EVENTTARGET'] == 'ctl00$cphRoblox$BigUglyAd$UserAdDisplay$AdImage') { ?>
			<script type="text/javascript">
